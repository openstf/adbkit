...
var Client = (function() {
  ...
  Client = class Client {
    ...
    getDHCPIpAddress(serial, iface, callback) {
      if (iface == null) { iface = 'wlan0'; }
    ...
    openMonkey(serial, port, callback) {
      ...
      return tryConnect(1)
        .catch(err => {
          return this.transport(serial)
            .then(transport => new MonkeyCommand(transport)
            .execute(port)).then(out => tryConnect(20)
            .then(monkey => monkey.once('end', () => out.end())));
      }).nodeify(callback);
    }
    ...
    openLogcat(serial, options, callback) {
      ...
      return this.transport(serial)
        .then(transport => new LogcatCommand(transport)
        .execute(options)).then(stream => Logcat.readStream(stream,
