...
export default class LineTransform extends Transform {
    ...
    constructor(options: TransformOptions & { autoDetect?: boolean } = {}) {
        const { autoDetect = false, ...transformOptions } = options;
        super(options);
        this.savedR = null;
    ...
    _nullTransform(chunk: Buffer, encoding: string, done: () => void) {
        this.push(chunk);
        done();
    }
